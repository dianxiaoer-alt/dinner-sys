...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function(exports){
  ...
  Class.prototype.pullData = function(curr){
    ...
    if(options.url){ //Ajax请求
      ...
      $.ajax({
        ...
        ,success: function(res){
          ...
          if(res[response.statusName] != response.statusCode){
            ...
            that.renderData(res, curr, res[response.countName].total), sort();
            options.time = (new Date().getTime() - that.startTime) + ' ms'; //耗时（接口请求+视图渲染）
          }
          that.setColsWidth();
          typeof options.done === 'function' && options.done(res, curr, res[response.countName].total);
        }
        ,error: function(e, m){
      ...
      res[response.countName].total = options.data.length;
      
      //记录合计行数据
      if(typeof options.totalRow === 'object'){
        res[response.totalRowName].total = $.extend({}, options.totalRow);
      }
...
/*************************************可动态更改********************************************/
  ...
  Class.prototype.renderData = function(res, curr, count, sort){
    var that = this
    ,options = that.config
    ,data = res[options.response.dataName].list || [] //列表数据
    ,totalRowData = res[options.response.totalRowName].total //合计行数据
    ,trs = []
    ,trs_fixed = []
    ,trs_fixed_r = []
    ...
    if(options.page){
      options.page = $.extend({
        ...
        ,count: count
        ,limit: options.limit
        ,limits: options.limits || [20,30,40,50,60,70,80,90,100]
        ,groups: 3
