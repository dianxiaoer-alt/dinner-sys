package com.dinner.wx.pay.utils;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONArray;
import com.alibaba.fastjson.JSONObject;

import com.dinner.commons.utils.HttpClient;
...
import com.dinner.wx.pay.constants.WXURL;

import lombok.extern.slf4j.Slf4j;
import com.dinner.wx.pay.card.CardBgColorEnum;
import com.dinner.wx.pay.redis.RedisKeyEnum;
...
public class WXUtils {
   ...
   /* @Autowired
    ...
    public JSONObject getJsapiAccessTokenByCode(String code) {
        ...
        try {
            ...
            String access_token_json = restTemplate.getForObject(WXURL.OAUTH_ACCESS_TOKEN_URL, String.class,
                    config.getAPP_ID(), config.getSECRET(), code);
            log.info("appid -> "+config.getAPP_ID()+",secret -> "+ config.getSECRET());
     ...
     * @return
     */
    public JSONObject refushAccessToken(String refresh_token){
        // 通过接口取得access_token
        String access_token_json= restTemplate.getForObject(MessageFormat.format(WXURL.FLUSH_ACCESS_TOKEN_URL, config.getAPP_ID(),refresh_token), String.class);
        log.info(access_token_json);
        if (StringUtils.isBlank(access_token_json)) {
            return null;
        }
        JSONObject jsonObject = JSON.parseObject(access_token_json);
    ...
    public JSONObject getJsapiUserinfo(String access_token, String openid) {
        ...
        try {
            ...
            HttpEntity<JSONObject> entity = new HttpEntity<>(json, this.getHttpHeadersUTF8JSON());


            String userinfo_json = HttpClient.sendPost(MessageFormat.format(WXURL.OAUTH_GET_USERINFO_URL, access_token,config.getAPP_ID()),null);

         //   JSONObject jsonObject = restTemplate.postForObject(MessageFormat.format(WXURL.OAUTH_GET_USERINFO_URL, access_token,WXPayConstants.APP_ID), entity,JSONObject.class );
    ...
    public JSONObject createCardActivity(String begin_time, String end_time, int gift_num, int max_partic_times_act,
        ...
        try {
            ...
            basic_info.put("activity_tinyappid", config.getAPP_ID());
            // mch_code	是	支付商户号
            basic_info.put("mch_code",config.getMCH_ID());
