...
public class WXPayReport {
    ...
    private WXPayRequestConfig wxPayRequestConfig;

    private volatile static WXPayReport INSTANCE;

    private WXPayReport(final WXPayRequestConfig wxPayRequestConfig,final WXPayConfig config) {
        this.config = config;
        this.wxPayRequestConfig = wxPayRequestConfig;
     ...
     * @return
     */
    public static WXPayReport getInstance(WXPayRequestConfig wxPayRequestConfig,WXPayConfig config) {
        if (INSTANCE == null) {
            synchronized (WXPayReport.class) {
                if (INSTANCE == null) {
                    INSTANCE = new WXPayReport(wxPayRequestConfig,config);
                }
            }
        }
        return INSTANCE;
    ...
    public void report(String uuid, long elapsedTimeMillis,
        ...
        ReportInfo reportInfo = new ReportInfo(uuid, currentTimestamp, elapsedTimeMillis,
                firstDomain, primaryDomain, firstConnectTimeoutMillis, firstReadTimeoutMillis,
                firstHasDnsError, firstHasConnectTimeout, firstHasReadTimeout);
        String data = reportInfo.toLineString(wxPayRequestConfig.getAPI_KEY());
